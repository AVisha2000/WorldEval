...
func _run() -> void:
	if DisplayServer.get_name() == "headless":
		_fail("embodiment_v3_movie_maker_environment_invalid")
		return
	var replay_path := _replay_path()
	if replay_path.is_empty():
		_fail("embodiment_v3_movie_maker_replay_missing")
		return
	...
	if payload.is_empty() or not bool(Verifier.new().verify(payload).get("ok", false)):
		_fail("embodiment_v3_movie_maker_replay_unverified")
		return
	var parsed := Codec.parse_canonical(payload, Verifier.MAX_REPLAY_BYTES)
	payload.fill(0)
	if not bool(parsed.get("ok", false)):
		_fail("embodiment_v3_movie_maker_replay_invalid")
		return
	...
	if _participant_id not in replay.config.participant_ids:
		_fail("embodiment_v3_movie_maker_participant_invalid")
		return
	var dispatcher := Dispatcher.new()
	if not dispatcher.configure(replay.config).is_empty():
		_fail("embodiment_v3_movie_maker_authority_rejected")
		return
	_scene = ParticipantScene.new()
	root.add_child(_scene)
	if not _scene.configure_task(
		str(replay.config.task_id), _participant_id, "", int(replay.config.seat_rotation)
	):
		_fail("embodiment_v3_movie_maker_projection_rejected")
		return
	...
	for step_value: Variant in replay.steps:
		...
		for frame: int in frame_count:
			...
			var source := ProjectionTween.interpolate(
				current_source, after_source, progress_milli, "llm-controller/0.3.0"
			)
	...
	for frame: int in OUTRO_FRAMES:
		await process_frame
	print("EMBODIMENT_V3_MOVIE_MAKER_OK")
	quit(0)
