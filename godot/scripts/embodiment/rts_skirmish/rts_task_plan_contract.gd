class_name EmbodimentRtsTaskPlanContract
extends RefCounted

## Additive milestone contract for rts-skirmish-v0.  Coordinates deliberately are not part of
## this wire shape: the authority expands a visible target into the physical route and hold.

const PROTOCOL := "rts-task-plan-v1"
const MAX_INTENT_BYTES := 160
const MAX_MEMORY_BYTES := 2048
const TASKS := ["gather", "return_material", "build", "arm", "rally", "attack_unit", "attack_structure", "retreat", "hold"]
