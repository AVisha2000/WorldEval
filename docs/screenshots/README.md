# WorldArena Screenshots

This page collects repository-owned captures of implemented WorldArena gameplay and interfaces.
Images are checked into `docs/screenshots` so README and documentation pages do not depend on
temporary render paths or third-party hosting.

## Mini RTS Skirmish

### Persistent resource economy

Workers walk to the pre-existing tree and ore fields, harvest for a visible duration, carry their
loads home, and unlock the rest of each three-unit faction.

![A Luna worker harvesting persistent resources](worldarena-rts-resource-gathering.jpg)

![Both RTS factions spreading across their resource fields](worldarena-rts-economy.jpg)

### Rally and bridge battle

The same workers become militia, form squads, march toward the bridge, and exchange animated hits
under public health bars, safe task summaries, and Terra/Luna agent-status panels.

![Terra militia marching toward the bridge](worldarena-rts-squad-march.jpg)

![Terra and Luna militia fighting on the bridge](worldarena-rts-bridge-battle.jpg)

### Retreat, siege, and victory

Luna's final fighter retreats, the two surviving Terra militia counterattack, and the match ends by
destroying Luna's tower and Town Hall.

![Terra pursuing Luna toward the stronghold](worldarena-rts-red-retreat.jpg)

![Two Terra survivors attacking Luna's Town Hall](worldarena-rts-town-hall-assault.jpg)

![Two Terra survivors celebrating the deterministic victory](worldarena-rts-blue-victory.jpg)

## Labyrinth Run

### Three-agent race

Sol, Luna, and Terra race physically separate copies of the same maze. The broadcast keeps the
model-colour key and live spatial-reasoning metrics visible throughout the run.

![Sol, Luna, and Terra racing identical labyrinths](worldarena-labyrinth-run-race.png)

### Verified winner calling card

The native broadcast closes with the final podium, winning strategy, finish time, travelled cells,
and path efficiency.

![Sol's verified Labyrinth Run winner calling card](worldarena-labyrinth-run-winner.png)

### Responsive Controller Lab

The same model key, winner rationale, and podium are available in the Controller Lab on narrow
screens.

![Labyrinth Run winner card in the mobile Controller Lab](worldarena-labyrinth-run-dashboard-mobile.jpg)

## Controller and marketing surfaces

### Simulation Lab

![WorldArena Simulation Lab](worldarena-simulation-lab-implemented.png)

### Artifact replay

![WorldArena artifact replay](worldarena-artifact-replay-implemented.png)

### Marketing site

![WorldEval marketing site](worldeval-marketing-infinity-final.png)
