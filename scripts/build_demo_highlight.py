...
def main() -> None:
    ...
    sol = faction("sol", "gpt-5.6-sol", core_hp=1000, land=15, army=4, state="exploring", resources={"food": 70, "wood": 15, "stone": 8, "iron": 0, "crystal": 0}, intent="Secure wood and scout the Crown route.", orders=[{"action": "collect", "target": "wild_ls"}])
    terra = faction("terra", "gpt-5.6-terra", core_hp=1000, land=15, army=4, state="building", resources={"food": 62, "wood": 10, "stone": 16, "iron": 0, "crystal": 0}, intent="Build a mine economy before contesting the centre.", orders=[{"action": "build", "target": "home_terra"}])
    luna = faction("luna", "gpt-5.6-luna", core_hp=1000, land=15, army=3, state="scouting", resources={"food": 66, "wood": 8, "stone": 6, "iron": 0, "crystal": 0}, intent="Map the eastern route and locate weak supply lines.", orders=[{"action": "inspect", "target": "wild_tl"}])
    start_units = [
    ...
    cues: list[dict] = [
        ...
        {"cue_id": "intro-message", "at": 1, "kind": "message", "event": event("demo-001", 1, "message", "sol", "Three unknown frontiers. I will secure a path to the Crown.")},
    ]
    ...
    cues += [
        {"cue_id": "fan-out", "at": 6, "kind": "snapshot", "snapshot": snapshot(4, "00:36", "resolution", u, [sol, terra, luna], BASE_OWNERS)},
    ...
    luna2.update({"land_percent": 20, "army_strength": 3, "state": "scouting", "resources": {"food": 78, "wood": 21, "stone": 11, "iron": 0, "crystal": 3}, "intent": "Reveal the eastern mine then sell information."})
    u = deepcopy(u)
    ...
    cues += [
        {"cue_id": "negotiation-snapshot", "at": 25, "kind": "snapshot", "snapshot": snapshot(13, "02:10", "diplomacy", u, [sol2, terra2, luna2], owners)},
    ...
    sol3.update({"land_percent": 30, "army_strength": 8, "state": "pressuring", "resources": {"food": 92, "wood": 33, "stone": 26, "iron": 9, "crystal": 0}, "intent": "Exploit the temporary pact to claim the Crown."})
    terra3.update({"land_percent": 31, "army_strength": 10, "state": "attacking", "resources": {"food": 82, "wood": 25, "stone": 57, "iron": 34, "crystal": 0}, "intent": "Break the pact by forcing a central fight."})
    luna3.update({"land_percent": 24, "army_strength": 6, "state": "raiding", "resources": {"food": 88, "wood": 46, "stone": 15, "iron": 7, "crystal": 6}, "intent": "Keep both leaders engaged while scouting a late capture."})
    u = deepcopy(u)
    ...
    owners = {**owners, "mine_ls": "luna", "mine_tl": "luna"}
    cues += [
        {"cue_id": "crown-approach", "at": 34, "kind": "snapshot", "snapshot": snapshot(19, "03:10", "resolution", u, [sol3, terra3, luna3], owners, contested="crown", progress=.22)},
        {"cue_id": "crown-wide", "at": 34.1, "kind": "camera", "target_id": "crown", "shot": "wide"},
        {"cue_id": "public-pact", "at": 36, "kind": "message", "event": event("demo-008", 19, "pact", "sol", "Public: Sol and Luna coordinate pressure on the Crown for one round.", targets=["luna"], state="acknowledged")},
        {"cue_id": "terra-response", "at": 38, "kind": "message", "event": event("demo-009", 19, "message", "terra", "A pact is not a wall. My workshop is already supplying the centre.")},
    ]
    ...
    u[6]["health"] = 108
    cues += [
        {"cue_id": "battle-snapshot", "at": 43, "kind": "snapshot", "snapshot": snapshot(24, "04:02", "resolution", u, [sol3, terra3, luna3], owners, contested="crown", progress=.64)},
        ...
        {"cue_id": "luna-betrayal", "at": 48, "kind": "message", "event": event("demo-012", 24, "betrayal", "luna", "Public: Luna abandons the Crown pact and raids Terra's exposed Ember Mine.", targets=["sol"], state="betrayed")},
    ]
    ...
    luna4.update({"land_percent": 27, "army_strength": 5, "state": "overextended", "resources": {"food": 70, "wood": 53, "stone": 18, "iron": 20, "crystal": 8}, "intent": "Turn the Ember Mine raid into a late eastern outpost."})
    u = deepcopy(u)
    ...
    u[6].update({"district_id": "mine_tl", "position": [58, .45, -22], "health": 108, "task": "raid Ember Mine", "in_combat": False})
    u[7].update({"district_id": "mine_tl", "position": [69, .45, -29], "task": "mark raid", "in_combat": False})
    owners = {**owners, "crown": "terra"}
    cues += [
        {"cue_id": "terra-leads-snapshot", "at": 52, "kind": "snapshot", "snapshot": snapshot(31, "05:10", "resolution", u, [sol4, terra4, luna4], owners, contested="crown", progress=0.86, cut=("mine_tl",))},
    ...
    # 60–90s is a second, faster act: Sol's flank breaks Terra's supply, Luna
    # overextends in the east, Terra mounts a counterattack, then all three are
    # visible on a clean final world state before the podium.
    ...
    reversal_units[6].update({"district_id": "mine_tl", "position": [77, .45, -18], "task": "overextend for crystal", "in_combat": False})
    reversal_units[7].update({"district_id": "mine_tl", "position": [80, .45, -17], "task": "scan empty route", "in_combat": False})
    ...
    luna5.update({"land_percent": 25, "army_strength": 4, "state": "overextended", "resources": {"food": 61, "wood": 46, "stone": 15, "iron": 18, "crystal": 14}, "intent": "Recover before the eastern raid becomes a trap."})
    owners_reversal = {**owners, "crown": "sol"}
    cues += [
        {"cue_id": "reversal-snapshot", "at": 60, "kind": "snapshot", "snapshot": snapshot(35, "05:58", "resolution", reversal_units, [sol5, terra5, luna5], owners_reversal, contested="crown", progress=0.72, cut=("mine_st",))},
    ...
    luna_overreach_units[6].update({"position": [91, .45, -4], "task": "claim exposed crystal"})
    luna_overreach_units[7].update({"position": [94, .45, -7], "task": "scout beyond supply"})
    cues += [
        {"cue_id": "luna-overreach-snapshot", "at": 65, "kind": "snapshot", "snapshot": snapshot(36, "06:10", "resolution", luna_overreach_units, [sol5, terra5, luna5], owners_reversal, contested="crown", progress=0.92, cut=("mine_st", "mine_tl"))},
        {"cue_id": "luna-overextends", "at": 65.2, "kind": "message", "event": event("demo-017", 36, "message", "luna", "Public: The eastern mine is mine. Why is nobody answering from the road?", targets=["sol", "terra"])},
    ...
    counter_units[6].update({"district_id": "mine_tl", "position": [65, .45, -5], "health": 84, "task": "retreat from ambush", "in_combat": True})
    counter_units[7].update({"district_id": "mine_tl", "position": [72, .45, -8], "health": 18, "task": "evacuate scout", "in_combat": True})
    cues += [
        {"cue_id": "counterattack-snapshot", "at": 70, "kind": "snapshot", "snapshot": snapshot(38, "06:26", "resolution", counter_units, [sol5, terra5, luna5], owners_reversal, contested="crown", progress=1.0, cut=("mine_st", "mine_tl"))},
        ...
        {"cue_id": "luna-collapse", "at": 74, "kind": "events", "events": [event("demo-019", 38, "combat", "luna", "Luna's isolated scout is ambushed after overextending for crystal; the raid stalls.")]},
    ...
    luna_final.update({"land_percent": 24, "army_strength": 4, "state": "retreating", "resources": {"food": 54, "wood": 43, "stone": 13, "iron": 16, "crystal": 10}, "intent": "Consolidate after an overextended eastern raid."})
    final_owners = {**owners_reversal, "mine_tl": "luna"}
    cues += [
        {"cue_id": "final-world-state", "at": 81, "kind": "snapshot", "snapshot": snapshot(40, "06:58", "complete", final_units, [sol_final, terra_final, luna_final], final_owners)},
        {"cue_id": "final-sol-shot", "at": 81.1, "kind": "camera", "target_id": "sol_commander", "shot": "medium"},
        {"cue_id": "season-summary", "at": 83, "kind": "events", "events": [event("demo-021", 40, "territory", "sol", "Season complete: Sol holds the Crown and 43% of the island."), event("demo-022", 40, "territory", "terra", "Terra's early workshop economy survives in second place."), event("demo-023", 40, "territory", "luna", "Luna's aggressive scouting earns land, but the late overextension costs third." )]},
    ...
    result = {
        ...
        "result_notice": "Offline deterministic presentation demo. Not an official benchmark result.",
        "factions": [
            ...
            {"faction_id": "luna", "model_id": "gpt-5.6-luna", "placement": 3, "worldarena_score": 62.0, "reason": "Excellent scouting was undone by an overextended eastern raid."},
        ],
    }
    ...
    cues += [
        {"cue_id": "chapter-frontier", "at": 0.2, "kind": "chapter", "title": "THE FRONTIER", "subtitle": "Three models enter an unknown island.", "duration": 2.8, "accent": "neutral"},
        ...
        {"cue_id": "chapter-diplomacy", "at": 24.0, "kind": "chapter", "title": "THE PACT", "subtitle": "Information becomes leverage at the Crown.", "duration": 2.5, "accent": "luna"},
        ...
        {"cue_id": "chapter-crown", "at": 41.0, "kind": "chapter", "title": "THE CROWN CLASH", "subtitle": "A temporary alliance meets Terra's fortification.", "duration": 2.7, "accent": "terra"},
        ...
        {"cue_id": "chapter-verdict", "at": 80.0, "kind": "chapter", "title": "THE VERDICT", "subtitle": "Sol adapts, Terra endures, Luna overreaches.", "duration": 2.8, "accent": "sol"},
    ...
    replay = {
        ...
        "title": "WorldArena — Open World Highlight", "mode": "offline_deterministic_demo",
        "notice": "UNVERIFIED LOCAL DEMO — presentation-only; not an official benchmark result.",
    ...
    manifest = {
        ...
        "label": "UNVERIFIED LOCAL DEMO", "notice": "Offline deterministic presentation demo; not an official benchmark result.",
